...
`mason` is an agent skills kit (`skills/`; `.tessl-plugin/plugin.json` retained for tessl packaging): `npx skills add canonical/mason --skill chisel-slicer`. the same tree installs via `gh skill install` and the claude code marketplace (`.claude-plugin/`) -- see README. there is no installer of our own any more; a skill has to be self-contained (hence `_shared/` + `sync-shared`). adding a capability = a new skill dir under `skills/`, name equal to the dir, `SKILL.md` frontmatter per agentskills.io.
